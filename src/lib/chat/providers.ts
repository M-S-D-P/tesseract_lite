...
const RUNTIME_TOOL_DESCRIPTION =
  "Query LIVE runtime telemetry streamed from the running Rails application: request volume, slowest controller actions, failing endpoints, N+1 query patterns, hottest SQL, and recent requests. action=coverage compares the indexed source against observed traffic: which controllers are exercised, which serve no traffic, and which serve traffic without appearing in the source at all. action=metaprogramming lists methods that DID execute but are not defined in the source file they came from — Rails generates methods at runtime (concerns, generated association and attribute methods, scopes, delegation, method_missing), so this is evidence reading the code cannot produce. Each finding carries an origin: appmap means an instrumented call event carrying the method's own Method#source_location, which covers EVERY method that ran including ones that touch no database; log means it was inferred from the source line under a SQL statement, so it only covers methods that were on the stack when a query ran. Prefer appmap findings, say which origin you are citing, and treat a not-indexed verdict as no claim either way. ALWAYS call action=metaprogramming when asked what is dynamically generated, what static analysis would miss, or how metaprogramming is handled — never answer those from general knowledge of Rails.";
